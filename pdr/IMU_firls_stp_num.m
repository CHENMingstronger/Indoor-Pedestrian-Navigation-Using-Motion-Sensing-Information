...
for i = 2 :len_y
    if dlt_y(i-1) > 0 && dlt_y(i) < 0 && y(i) > ap %peak
        pk_num = pk_num + 1;
        pk_acc(pk_num) = y(i);
        pk_pos(pk_num) = i;
        pk_flg(i) = y(i);
    ...
    if dlt_y(i-1) < 0 && dlt_y(i) > 0 && y(i) < ap*(-1) %vally
        vy_num = vy_num + 1;
        vy_acc(vy_num) = y(i);
        vy_pos(vy_num) = i;
        vy_flg(i) = y(i);
...
xlabel('\fontname{Songti SC}加速度数');ylabel('\fontname{Songti SC}振幅');
title('\fontname{Songti SC}基于滤波器的步频检测');
legend('\fontname{Songti SC}波峰','\fontname{Songti SC}波谷','\fontname{Songti SC}三轴加速度矢量和');
