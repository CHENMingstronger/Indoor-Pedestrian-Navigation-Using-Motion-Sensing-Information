...
function [height,stp_ori,STP_ORI, pk_pos, vy_pos,NE, myaw] = IMU_firls_stp_ori_loc(stp_num, stp_len, py_flg, yaw,NE_ini, pk_index)
%{
%   INPUTS
...
stp_ori = zeros(stp_num, 1); %每步的方向

height_num = 0; %初始化高度
stair_height = -0.15;  %每阶阶梯高度设定
height = zeros(stp_num, 1);  %高度计量
...
%          sum_gyr(i) = sum(gyrz(pk_pos(i)+1:vy_pos(i)).*diff(spl_time(pk_pos(i):vy_pos(i))));
    ...
    %忽略微小的航向偏差
    if i >=2
       if abs(myaw(i)/deg2rad - myaw(i-1)/deg2rad) < 0
           myaw(i) = myaw(i-1);
       end
    end
...
%加高度并通过阈值进行判断是否进入转弯平台
    if pk_index(i) > 1.9
    height_num = height_num + stair_height;
    height(i) = height_num;
